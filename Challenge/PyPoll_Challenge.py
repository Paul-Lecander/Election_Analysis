...
with open(file_to_load) as election_data:
    ...
    for row in reader:
        ...
        if counties not in county_list:

            # 4b: Add the existing county to the list of counties.
            county_list.append(counties)

            # 4c: Begin tracking the county's vote count.
            county_votes_dict[counties] = 0

        # 5: Add a vote to that county's vote count.
        county_votes_dict[counties] += 1
...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_votes_dict:
        ...
        county_vote_percentage = float(county_votes) / float(total_votes) * 100
         ...
         # 6d: Print the county results to the terminal. (Print statement includes Current County, % of Total Votes,
         # AND its Total Votes to the command line)
        ...
        txt_file.write(current_county_results)        

         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (county_votes > county_votes_largest_turnout) and (county_vote_percentage > county_winning_percentage):
            county_largest_turnout = county
            county_winning_percentage = county_vote_percentage
            county_votes_largest_turnout = county_votes    
